...
public class WalletAppKit extends AbstractIdleService {
    ...
    final static String TAG = "WalletAppKit.java";
    @Override
    ...
    private Wallet createOrLoadWallet(boolean shouldReplayWallet) throws Exception {
        ...
        if (vWalletFile.exists()) {
            ...
            wallet = createWallet();
            Log.i(TAG, "Freshing Receive key");
            wallet.freshReceiveKey();
            Log.i(TAG,"createOrLoadWallet() is done doing Freshing keys");
            for (WalletExtension e : provideWalletExtensions()) {
                wallet.addExtension(e);
            }
            Log.i(TAG,"createOrLoadWallet() is done parsing extensions");
            ...
            Log.i(TAG, "createOrLoadWallet() is done saving to file");
            wallet = loadWallet(false);
            Log.i(TAG, "createOrLoadWallet() is done loadingWallet(false)");
        }

        if (useAutoSave) wallet.autosaveToFile(vWalletFile, 5, TimeUnit.SECONDS, null);
        Log.i(TAG,"createOrLoadWallet() returning wallet");
        return wallet;
    }
