...
public class PeerGroup implements TransactionBroadcaster {
        ...
        public void go() {
            ...
            try {
                ...
                if (!Utils.isAndroidRuntime() && useLocalhostPeerWhenPossible && maybeCheckForLocalhostPeer() && firstRun) {
                    log.debug("Localhost peer detected, trying to use it instead of P2P discovery");
                    maxConnections = 0;
                    connectToLocalHost();
                    return;
                }
            ...
            try {
                ...
                if (inactives.isEmpty()) {
                    if (countConnectedAndPendingPeers() < getMaxConnections()) {
                        log.debug("Peer discovery didn't provide us any more peers, will try again later.");
                ...
                if (retryTime > now) {
                    ...
                    log.debug("Waiting {} msec before next connect attempt {}", delay, addrToTry == null ? "" : "to " + addrToTry);
    ...
    protected int discoverPeers() throws PeerDiscoveryException {
        ...
        log.debug("Peer discovery took {}ms and returned {} items", System.currentTimeMillis() - start,
                addressList.size());
    ...
    public void stop() {
        try {
            stopAsync();
            log.debug("Awaiting PeerGroup shutdown ...");
    ...
    protected void handleNewPeer(final Peer peer) {
        ...
        try {
            ...
            peers.add(peer);
            newSize = peers.size();
            log.debug("{}: New peer      ({} connected, {} pending, {} max)", peer, newSize, pendingPeers.size(), maxConnections);
    ...
    private void setDownloadPeer(@Nullable Peer peer) {
        ...
        try {
            if (downloadPeer == peer)
                return;
            if (downloadPeer != null) {
                log.debug("Unsetting download peer: {}", downloadPeer);
            ...
            downloadPeer = peer;
            if (downloadPeer != null) {
                log.debug("Setting download peer: {}", downloadPeer);
                if (downloadListener != null)
    ...
    @GuardedBy("lock") private int stallMinSpeedBytesSec = Block.HEADER_SIZE * 10;// was 20 not 10
            ...
            synchronized (this) {
                ...
                if (!syncDone) {
                    if (warmupSeconds < 0) {
                        ...
                        log.debug(String.format("%d blocks/sec, %d tx/sec, %d pre-filtered tx/sec, avg/last %.2f/%.2f kilobytes per sec (stall threshold <%.2f KB/sec for %d seconds)",
                        ...
                        if (bytesInLastSecond > 0)
                            log.debug(String.format("%d blocks/sec, %d tx/sec, %d pre-filtered tx/sec, last %.2f kilobytes per sec",
